...
import {
  Paintbrush, Monitor, Save, RefreshCw, CheckCircle2, RotateCcw,
...
export function ProfileBuilder({ slug, initialTokens }: Props) {
  ...
  return (
    <div className="flex gap-5" style={{ height: "calc(100vh - 8.5rem)" }}>
      {/* ── Left: controls ── */}
      <div className="w-72 shrink-0 flex flex-col bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-white/5 rounded-2xl overflow-hidden">
        {/* Header */}
        <div className="flex items-center gap-3 px-4 py-3 border-b border-zinc-200 dark:border-white/5">
          <Link
            href="/dashboard/profile"
            className="p-1.5 rounded-lg text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-white/5 transition-all"
          >
            <ArrowLeft size={14} />
          </Link>
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-sm font-extrabold text-zinc-900 dark:text-white leading-tight">Design Studio</h2>
            <p className="text-[10px] text-zinc-500 truncate">Live preview updates as you edit</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="font-bold text-zinc-900 dark:text-white">Live Preview</span>
            <span className="text-zinc-400 text-xs">· auto-refreshes on change</span>
          </div>
          <a
            href={`/u/${slug}`}
            target="_blank"
            className="flex items-center gap-1.5 text-xs font-bold text-indigo-500 hover:text-indigo-400 transition-colors"
          >
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-3">
      <p className="text-[10px] font-black text-zinc-500 dark:text-zinc-400 uppercase tracking-[0.15em]">{title}</p>
  ...
  return (
    <div className="flex items-center gap-3">
      <label className="relative cursor-pointer shrink-0">
        ...
        />
        <div
          className="w-8 h-8 rounded-lg shadow-sm ring-1 ring-zinc-200 dark:ring-white/10 transition-transform hover:scale-110"
