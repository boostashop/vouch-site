...
import { getBotGuildChannels } from "./actions"
...
export function DiscordChannelSelector({
  ...
  const handleGuildChange = async (guildId: string) => {
    ...
    startTransition(async () => {
      try {
        const fetchedChannels = await getBotGuildChannels(guildId)
        setChannels(fetchedChannels)
        if (fetchedChannels.length > 0) {
          // Default to first channel in list
  ...
  return (
    <div className={`space-y-4 md:col-span-3 grid md:grid-cols-2 gap-4 ${!isPremium ? "opacity-50 pointer-events-none" : ""}`}>
